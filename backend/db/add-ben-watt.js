/**
 * One-off script: find Ben Watt in Trainerize, add him to the portal as
 * my_fit_coach, and backfill the last 2 years of data.
 *
 * Usage: node backend/db/add-ben-watt.js
 */
...
async function rateLimitedPost(endpoint, body) {
  ...
  requestCount++;
  return trainerizePost(endpoint, body, { label: 'AddBenWatt', useCache: false });
}
...
// ---------------------------------------------------------------------------
// Find Ben Watt in the active client list
// ---------------------------------------------------------------------------
async function findBenWatt() {
  let start = 0;
  while (true) {
...
// ---------------------------------------------------------------------------
async function main() {
  console.log('=== Add Ben Watt ===\n');
  console.log(`Backfill range: ${BACKFILL_START} → ${BACKFILL_END}\n`);

  // 1. Find Ben Watt in Trainerize
  console.log('Searching Trainerize active clients for Ben Watt...');
  const benUser = await findBenWatt();
  if (!benUser) {
    console.error('ERROR: Could not find "Ben Watt" in the active client list. Make sure he is active in Trainerize.');
    process.exit(1);
  }
  const tid = String(benUser.id);
  const email = (benUser.email || '').toLowerCase() || null;
  console.log(`Found: ${benUser.firstName} ${benUser.lastName} | trainerize_id=${tid} | email=${email || '(none)'}\n`);
  ...
  if (existing.rows.length > 0) {
    ...
    // Re-activate and ensure correct program
    await pool.query(
      `UPDATE clients SET active=true, program='my_fit_coach', trainerize_joined_at=$1
       WHERE id=$2`,
      [BACKFILL_START, clientId]
    );
    console.log(`Client already exists (id=${clientId}) - re-activated and set program=my_fit_coach.\n`);
  } else {
    const ins = await pool.query(
      `INSERT INTO clients (coach_id, trainerize_id, name, email, program, active, pending_setup, trainerize_joined_at)
       ...
       RETURNING id`,
      [COACH_ID, tid, `${benUser.firstName} ${benUser.lastName}`.trim(), email, BACKFILL_START]
    );
    clientId = ins.rows[0].id;
    console.log(`Inserted Ben Watt as portal client id=${clientId}.\n`);
  }
  ...
  console.log('\n=== Done. Ben Watt is live on the dashboard. ===');
  await pool.end();
}
